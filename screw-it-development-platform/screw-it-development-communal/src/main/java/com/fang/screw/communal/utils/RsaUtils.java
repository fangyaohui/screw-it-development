...
public class RsaUtils {
     ...
     * @param privateKey    私钥(BASE64编码)
     */
    public static <T> T decryptByPrivateKey(String encryptedData,Class<T> clazz) throws Exception {
        byte[] encryptedByte = java.util.Base64.getDecoder().decode(encryptedData);
        //base64格式的key字符串转Key对象
        ...
        //分段进行解密操作
        encryptedByte = encryptAndDecryptOfSubsection(encryptedByte, cipher, MAX_DECRYPT_BLOCK);
        return fromJson(Base64.encodeBase64String(encryptedByte),clazz);
    }

    // 泛型方法，接收JSON字符串和类类型，返回该类的对象
